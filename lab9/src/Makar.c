...
void ErrorBadNumber() { printf("bad number of lines"); }
...
void FreeArr(char* visitArr, long long int* pathLengthArr, Graph graph,
             int* printArr) {
  free(graph.Matrix);
  free(visitArr);
  free(pathLengthArr);
...
void OutputShortestPath(const long long* pathLengthArr, const int count,
                        const int start, const int finish, int* printArr) {
...
void OutputDistancesEachVertex(const Graph graph,
                               const long long* pathLengthArr) {
...
int GetEdgeIndex(const int edgeBegin, const int edgeEnd, const Graph graph) {
  return (2 * graph.NumVert - edgeBegin + 1) * edgeBegin / 2 + edgeEnd -
         edgeBegin;
}

int SearchIndex(const int curValueFrom, const int curValueTo,
                const Graph graph) {
...
int FindSource(const long long* pathLengthArr, const char* visitArr,
               const Graph graph) {
  int indexSource = 0;
...
void FindShortestPath(Graph graph, int indexSource, long long* pathLengthArr,
                      char* visitArr, int* count, int* printArr) {
  for (int i = 0; i < graph.NumVert; ++i) {
    int index = SearchIndex(indexSource, i, graph);
    if (graph.Matrix[index] != 0 && visitArr[i] == 0) {
      long long curLength =
          (long long)graph.Matrix[index] + pathLengthArr[indexSource];
...
bool Input(Graph graph) {
  ...
  for (int i = 0; i < M; ++i) {
    ...
    if (!CheckValue(curValueTo, graph.NumVert) ||
        !CheckValue(curValueFrom, graph.NumVert)) {
      return false;
    }
